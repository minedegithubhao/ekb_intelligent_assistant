...
def main() -> None:
    ...
    for batch_no, markdown_batch in enumerate(batches(markdown_paths, chunk_batch_size), start=1):
    ...
    total_faq_records = 0
    if faq_paths:
        faq_batch = pipeline.ingest(
            markdown_paths=[],
            faq_paths=faq_paths,
            index_csv_path=index_csv_path,
        )
        total_faq_records = len(faq_batch.faq_records)
        print(
            {
                "stage": "faq",
                "faq_records": total_faq_records,
                "faq_files": [path.name for path in faq_paths],
            }
        )
